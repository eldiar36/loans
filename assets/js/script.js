...
/*let stepsTwo = [false, false,  false, false,false,false];
let curr_stepTwo = 0;
function to_stepTwo(index, need_pushTwo) {
    curr_stepTwo = index;
    for (var i = 0; i < stepsTwo.length; i++) {
        if (!$("#step" + i).is(':hidden')) {
            $("#step" + i).hide();
        }
    }
    ...
    $("#progress_in-two").css({width: (100 * index / stepsTwo.length) + "%"});
    $("#curr_step-two").text("Шаг " + index + " из " + (stepsTwo.length - 1));

    // Разделение на #step0, #other_steps и #last_step
    if (index + 1 == stepsTwo.length) { // если шаг равен общему количеству шагов
        if (!$("#other_steps-two").is(':hidden')) {
            $("#other_steps-two").hide();
            $("#other_steps-two").show();
        }
    } else if (index > 0) { // если шаг больше ноля
        if ($("#other_steps-two").is(':hidden')) {
            $("#other_steps-two").show();
    ...
    } else if (!$("#other_steps-two").is(':hidden')) { // если шаг равен нолю
        $("#other_steps-two").hide();
    }
}
...
// Проверка заполненности радиокнопки или чекбокса
function check_radio_selectedTwo(elem_id, error_message) {
    objTwo = $('input[name="' + elem_id + '"]:checked');
    if (!(objTwo.length && objTwo.val())) {
        alert(error_message);
        return false;
    }
    return true;
...
(function ($) {
    ...
    $("#to_step2-two").click(function (event) {
        event.preventDefault();
        if (check_radio_selected("type-home-two", "Укажите Сумму")) {
            to_stepTwo(2, true);
        }
    });
    $("#to_step3-two").click(function (event) {
        event.preventDefault();
        var checked = $("#type-repair-two input:checked").length > 0;
        if (!checked) {
            alert("Укажите возраст ребенка");
            return false;
        } else {
    ...
    $("#to_step5-two").click(function (event) {
        event.preventDefault();
        if (check_radio_selected("design-project-two", "Укажите цель?")) {
            to_stepTwo(5, true);
        }
    });
    // Отправка формы (нажатием на финальную кнопку)
    $("#to_submit-two").click(function (event) {
        event.preventDefault();
        $("#quiz_form-two").submit();
    });
    // Проверка телефона и ПК при отправке формы
    $('#quiz_form-two').submit(function () {
        var name = $.trim($(this).find('input[name="name_input"]').val());
        var phone = $.trim($(this).find('input[name="phone"]').val());
        if (name === '') {
            alert('Заполните поле с именем');
...
$(document).ready(function () {
    ...
    jQuery('body').on('change', '#quiz_form-two', function () {
